...
If `git status --short` shows any changes after `vp fmt`, the working tree had pending format drift. Commit it on `main` *before* starting the worktree to keep it out of phase-6.5's no-ff range (lesson MS1.7):
...
describe("parseKv", () => {
  ...
  it("throws UsageError when an entry has no `=`", () => {
    ...
    expect(() => parseKv(["broken"], "--input")).toThrow(
      "invalid --input 'broken': expected k=v",
    );
  });

  it("throws UsageError when the key segment is empty", () => {
    expect(() => parseKv(["=value"], "--input")).toThrow(UsageError);
    expect(() => parseKv(["=value"], "--input")).toThrow(
      "invalid --input '=value': expected k=v",
    );
  });
