...
describe.skipIf(!POSIX)("runJob — deferred features", () => {
  test("job-level uses raises OrchestrationError", async () => {
    const job = parseJob({ uses: "ns/workflow@1.0.0" });
    await expect(runJob(makeRequest(job))).rejects.toBeInstanceOf(OrchestrationError);
  });

  test("if: expression with operators raises ExpressionError", async () => {
    const job = parseJob({
  ...
  test("job.outputs referencing steps.* raises ExpressionError", async () => {
    const job = parseJob({
      outputs: { result: "${{ steps.x.outputs.y }}" },
      steps: [{ id: "x", run: "echo x" }],
    });
    await expect(runJob(makeRequest(job))).rejects.toBeInstanceOf(ExpressionError);
  });
