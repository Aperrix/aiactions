 ...
 *
 * Scope (MS1.0 + MS1.0.5):
 * - `run:` steps only — `uses:` raises `OrchestrationError`.
 ...
 *   another unsupported context surfaces an `ExpressionError`.
 *
...
import { evaluateExpression } from "@aiactions/expression";
import type { EvalContext, StepOutputContext } from "@aiactions/expression";
import { writeScript } from "@aiactions/exec";
import { getShellInvocation } from "@aiactions/exec";
import { spawnShell } from "@aiactions/exec";
import { resolveUsesRef } from "./resolve-uses.ts";
import { executeUsesStep } from "@aiactions/exec";
import { OrchestrationError } from "../errors.ts";
import type { RuntimeEvent } from "@aiactions/schema";
import type { JobResult, RunStatus, StepResult } from "@aiactions/schema";
 ...
 * `<context>.<key>` body grammar surfaces an `ExpressionError`,
 ...
 * @throws {OrchestrationError} when the job uses a feature not
 *   yet implemented (e.g. job-level `uses:`, schema invariant violated).
 * @throws {ExpressionError} when an `${{ }}` body cannot be
...
export async function runJob(request: JobRunRequest): Promise<JobResult> {
  ...
  // `steps.*.outputs.*` will surface as an `ExpressionError`.
