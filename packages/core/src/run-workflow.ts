 ...
 *   `OrchestrationError` (deferred to MS1.1 / MS1.4).
...
import type { EvalContext } from "@aiactions/expression";
import { probeBashAvailability } from "@aiactions/exec";
import { runJob } from "./runner/run-job.ts";
import type { RunOptions, WorkflowInputValue } from "./run-options.ts";
import type { JobResult, RunResult, RunStatus } from "@aiactions/schema";
 ...
 * an `ExpressionError` (`inputs.X is not defined`) at evaluation
 ...
 *   `parseWorkflow` from `@aiactions/parser`. The schema's
