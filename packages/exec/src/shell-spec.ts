 ...
 *
 * @throws {ExecError} if `{0}` is not present (defensive
...
export function parseCustomShellTemplate(template: string): ParsedCustomShellTemplate {
  ...
  if (placeholderIdx === -1) {
    throw new ExecError(`shell template missing {0}: ${template}`);
  }
  const bin = tokens[0]!;
 ...
 * @throws {ExecError} when (a) `shell === 'sh'` on
...
export function getShellInvocation(
  ...
  if (typeof shell === "string" && !(BUILTIN_SHELLS as readonly string[]).includes(shell)) {
    if (!customShellTemplateRegex.test(shell)) {
      throw new ExecError(`shell value is neither a built-in nor a valid template: ${shell}`);
    }
  ...
  if (shell === "sh" && platform === "win32") {
    throw new ExecError("shell: sh is not available on Windows");
  }
  if (shell === "cmd" && platform !== "win32") {
    throw new ExecError("shell: cmd is only available on Windows");
  }
