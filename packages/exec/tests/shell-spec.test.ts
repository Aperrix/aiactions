...
import { getShellInvocation } from "../src/shell-spec.ts";
...
describe("getShellInvocation — rejection paths", () => {
  test("shell: sh on win32 throws ExecError", () => {
    expect(() => getShellInvocation("sh", SCRIPT, "win32", true)).toThrow(ExecError);
  });

  test("shell: cmd on linux throws ExecError", () => {
    expect(() => getShellInvocation("cmd", SCRIPT, "linux", true)).toThrow(ExecError);
  });

  test("shell: cmd on darwin throws ExecError", () => {
    expect(() => getShellInvocation("cmd", SCRIPT, "darwin", true)).toThrow(ExecError);
  });
});
